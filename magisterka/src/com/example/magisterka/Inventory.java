//do zrobienia:
//rozstawienie i dopasowanie wszystkiego od ekranu
...
public class Inventory extends Activity {
	...
	public TableLayout CreateItemsTable(int width, int height){   
        ...
        for(;;){
        	...
        	for(int j = 0; j < itemsInRow; j++){
        		...
        		rl.setBackgroundResource(R.drawable.inventory_frame);		
	...
	public void SaveInventoryInDatabase(){//do orania - najpierw kasuje potem zapisuje nowe rowy w bazie
		...
		while(i.hasNext()){
			...
			//newValues.put("_id", item.Get_id());//do orania - czy potrzebne jest to samo id co by³o wczeœniej?
	...
	class MyDragListener implements OnDragListener {
		...
		public boolean onDrag(View view, DragEvent event){
		    ...
		    case DragEvent.ACTION_DROP://do orania - za du¿o rzutowania (InventoryItem)View
		    	...
		    	//jesli na nowym miejscu coœ sie znajduje trzeba zamienic przedmioty
	...
	class MyLongClickListener implements OnLongClickListener{
		...
		public boolean onLongClick(View view)
		{
			...
			List<View> listInvisible = new ArrayList<View>();
			Iterator<View> itr;
			
			//znajdywanie gdzie moze znajdowac sie item
			if(availableSlot < bodyItems){//item do body
			    itr = listGadgetsSlot.iterator();
			    while(itr.hasNext()) {
			    	listInvisible.add(itr.next());
			    }
			    itr = listBodySlot.iterator();
			    while(itr.hasNext()) {
			    	View tmp = itr.next();
			    	if((Integer)tmp.getTag() != availableSlot + maxItemsBackpack)
			    		listInvisible.add(tmp);
			    }
			...
			//zaciemnianie ca³ej listy slotow
			itr = listInvisible.iterator();
			while(itr.hasNext()) {
				View _view = itr.next();
				_view.setAlpha(slotAlphaWhenDrag);
